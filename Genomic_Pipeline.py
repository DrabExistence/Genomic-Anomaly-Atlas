...
# NEW FONT FUNCTIONS START
def get_font_path(font_type="regular"):
    """Определяет путь к шрифту внутри папки проекта или в системе."""
    base_path = os.path.dirname(os.path.abspath(__file__))
    
    if font_type == "regular":
        local_font_name = "DejaVuSans.ttf"
        system_font_name = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
    elif font_type == "bold":
        local_font_name = "DejaVuSans-Bold.ttf"
        system_font_name = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"
    else:
        return None # Invalid font_type

    local_font_path = os.path.join(base_path, "assets", local_font_name)
...
def register_fonts():
    """Регистрирует шрифты для использования в PDF и возвращает их имена."""
    regular_font_name_colab = 'DejaVuSansRegular' # Distinct name for registration
    bold_font_name_colab = 'DejaVuSansBold' # Distinct name for registration
    
    reg_path = get_font_path("regular")
    bold_path = get_font_path("bold")

    if reg_path and bold_path:
        try:
            pdfmetrics.registerFont(TTFont(regular_font_name_colab, reg_path))
            pdfmetrics.registerFont(TTFont(bold_font_name_colab, bold_path))
            print(f"✅ Шрифты DejaVuSans успешно зарегистрированы.")
            return regular_font_name_colab, bold_font_name_colab
        except Exception as e:
            print(f"⚠️ Ошибка при регистрации шрифтов DejaVuSans: {e}. Используется Helvetica.")
            return 'Helvetica', 'Helvetica-Bold' # Fallback
    
    print("⚠️ Шрифт DejaVuSans не найден. Используется стандартный Helvetica (без поддержки кириллицы).")
    return 'Helvetica', 'Helvetica-Bold' # Fallback
# NEW FONT FUNCTIONS END
...
def save_report_to_pdf(results, dna_sequence, scan_data, exon_results, filename):
    font_regular_current, font_bold_current = register_fonts()
